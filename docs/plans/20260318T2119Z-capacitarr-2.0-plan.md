...
> **Status:** ✅ Complete — revised 2026-03-23 to reflect post-implementation removals
> **Created:** 2026-03-18
>
> ### Post-Implementation Revisions
>
> The following features were implemented during Phases 4–6 but subsequently removed after testing because they produced poor results or had critical bugs:
>
> | Feature | Removed In | Reason |
> |---------|-----------|--------|
> | Insights page (`/insights`) with Overview, Quality, and Watch Intelligence tabs | `1d65461` | Charts produced low-value visualizations; page felt bolted-on rather than integrated |
> | Bloat Detection (`QualityBloatFactor`, `/analytics/bloat`, "Bloated" smart filter) | `bcf3c31` | Size anomaly detection was inaccurate — median-based thresholds produced too many false positives |
> | Per-integration threshold overrides (`ThresholdPct`/`TargetPct` on `IntegrationConfig`) | `529acfa` | Critical bug: zero-value bool binding silently disabled integrations; library-level thresholds are sufficient |
> | Custom scoring weights UI | `529acfa` | Local-only state that was never persisted; confused users with "Preview — coming soon" badge |
> | Global rules (rules without an integration) | `36956f6` | Every rule must belong to an integration for scope clarity |
>
> Retained features from this plan that still ship: Dead/Stale content smart filters (on Library page), capacity forecast API, library entity with per-library thresholds, all architectural refactors. The Analytics/Insights features may be revisited in a future minor release with a simpler, dashboard-integrated approach.
