...
const numericFields = new Set([
  'rating', 'sizebytes', 'timeinlibrary', 'year',
  'seasoncount', 'episodecount', 'playcount', 'requestcount', 'lastplayed'
])
...
function numericToRange(op: string, val: number): [number, number] {
  ...
  switch (op) {
    case '==': return [val, val]
    case '!=': return [NEG_INF, INF]
    case '>': return [val + 0.001, INF]
    case '>=': return [val, INF]
    case '<': return [NEG_INF, val - 0.001]
